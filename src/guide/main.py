...
class ThermalMonitor:
            ...
            try:
                ...
                if temp is not None:
                    ...
                    if failure_count >= max_failures:
                        ...
                        self.logger.warning(f"Temperature read failures, increasing interval to {adjusted_interval}s")
    ...
    def _check_thermal_thresholds(self, avg_temp: float) -> None:
        ...
        if avg_temp >= self.halt_threshold and not self.is_halted:
            ...
            self.logger.critical(f"THERMAL HALT: {avg_temp:.1f}°C >= {self.halt_threshold}°C", extra={
                "thermal_event": "halt",
            ...
            self.is_throttled = True
            self.logger.warning(f"THERMAL THROTTLE: {avg_temp:.1f}°C >= {self.alert_threshold}°C", extra={
            ...
            self.logger.info(f"THERMAL RESUME: {avg_temp:.1f}°C <= {self.resume_threshold}°C", extra={
...
def create_app() -> FastAPI:
    ...
    if thermal_monitor.thermal_zone_path:
        logger.info("Thermal monitoring enabled", extra={
