...
class ModelManager:
    ...
    def validate_model(self, file_path: Path, expected_hash: Optional[str] = None) -> Dict[str, Any]:
        ...
        self.logger.info(f"Model validation successful: {file_path.name}", extra={
    ...
    def download_model(self, url: str, model_name: Optional[str] = None, 
        ...
        self.logger.info(f"Starting model download: {url}", extra={
        ...
        try:
                        ...
                        if chunk:  # Filter out keep-alive chunks
                            ...
                            if current_time - last_log_time >= 30:
                                if total_size:
                                    ...
                                    self.logger.info(f"Download progress: {progress:.1f}%", extra={
                                        "downloaded_mb": round(downloaded_size / (1024 * 1024), 2),
                                        "total_mb": round(total_size / (1024 * 1024), 2),
                                        "model_name": model_name
                                    })
                                else:
                                    self.logger.info(f"Downloaded: {downloaded_size / (1024 * 1024):.2f} MB", extra={
                                        "model_name": model_name
                                    })
            ...
            self.logger.info(f"Model download completed: {model_name}", extra={
                "file_size_mb": validation_result["file_size_mb"],
