...
def populated_system(client):
    ...
    test_documents = [
        {
            "source": "test_content.txt",
            "source_type": "file",
            "content": "The Local RAG system is designed for privacy and local operation."
        },
        {
            "source": "test_faq.txt", 
            "source_type": "file",
            "content": "Frequently asked questions about the system include performance and setup."
        }
    ]
...
class TestQueryStreamingResponse:
    ...
    def test_streaming_response_structure(self, populated_system):
        ...
        # Arrange
        request_data = {
            "query": "Explain the architecture of the Local RAG system"
        }
...
class TestQueryBusinessLogic:
    ...
    def test_context_retrieval_accuracy(self, populated_system):
        ...
        # Should retrieve relevant sources
        sources = response_data["sources"]
        assert len(sources) > 0
        
        # At least one source should mention privacy (basic relevance check)
        privacy_mentioned = any("privacy" in source["content"].lower() for source in sources)
...
class TestQueryErrorHandling:
    ...
    def test_system_not_initialized_error(self, client):
        """Test query when system components are not initialized."""
        # Note: This test simulates the condition where LLM or vector store is not ready
        # In practice, this would require mocking or specific test setup
        ...
        assert response.status_code == 500  # Configuration error
        
        response_data = response.json()
        assert "error" in response_data
        assert "ConfigurationError" in response_data["error"]
