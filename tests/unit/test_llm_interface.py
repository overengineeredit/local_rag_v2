...
class TestLLMInterface:
    ...
    def test_auto_detect_threads(self, mock_llama):
        ...
        with patch("os.cpu_count", return_value=8):
            llm = LLMInterface("/path/to/model.gguf", n_threads=None)
...
class TestQueryFromDictEdgeCases:
    ...
    def test_query_from_dict_none_created_at(self):
        ...
        assert (datetime.now(timezone.utc) - query.created_at).total_seconds() < 5
...
class TestLLMInterfaceQueryProcessing:
    ...
    def test_process_query_success(self):
        ...
        with patch.object(LLMInterface, '_load_model'):
            ...
            with patch.object(llm, 'generate_complete', return_value="AI is artificial intelligence.") as mock_generate:
    ...
    def test_process_query_with_context(self):
        ...
        with patch.object(LLMInterface, '_load_model'):
            ...
            query = Query(
                ...
                context_documents=[
                    {"content": "Machine learning is a subset of AI.", "metadata": {}, "distance": 0.1},
                    {"content": "It uses algorithms to learn patterns.", "metadata": {}, "distance": 0.2}
                ]
            )
            
            with patch.object(llm, 'generate_complete', return_value="ML is about learning from data.") as mock_generate:
                result = llm.process_query(query)
    ...
    def test_validate_context_length_warning_empty_context(self):
        ...
        with patch.object(LLMInterface, '_load_model'):
            ...
            with patch('guide.config.get', return_value=100):  # Small context window
                with patch.object(llm, 'estimate_tokens', side_effect=lambda text: len(text.split())):
                    result_context, was_truncated = llm.validate_context_length(very_long_query, short_context)
    ...
    def test_validate_context_length_truncation(self):
        ...
        with patch.object(LLMInterface, '_load_model'):
            ...
            with patch('guide.config.get', return_value=500):  # Small context window
                with patch.object(llm, 'estimate_tokens', side_effect=lambda text: len(text.split())):
                    result_context, was_truncated = llm.validate_context_length(short_query, very_long_context)
    ...
    def test_validate_context_length_no_truncation_needed(self):
        ...
        with patch.object(LLMInterface, '_load_model'):
            ...
            with patch('guide.config.get', return_value=2048):  # Large context window
                with patch.object(llm, 'estimate_tokens', side_effect=lambda text: len(text.split())):
                    result_context, was_truncated = llm.validate_context_length(short_query, short_context)
